...
describe("getWebsite", () => {
  ...
  it("should return the cleaned URL for a LinkedIn site", async () => {
    process.env.LINKEDIN_EMAIL = "dymmy@gmail.com";
    process.env.LINKEDIN_PASSWORD = "dummy123!";
    ...
    expect(mockPage.type).toHaveBeenCalledWith("#username", "dymmy@gmail.com");
    expect(mockPage.type).toHaveBeenCalledWith("#password", "dummy123!");
    expect(mockPage.click).toHaveBeenCalledWith(
      'button.btn__primary--large.from__button--floating[data-litms-control-urn="login-submit"]'
    );
    expect(mockPage.waitForNavigation).toHaveBeenCalledTimes(1);
    expect(mockPage.goto).toHaveBeenCalledWith(`${url}/about/`, {
      waitUntil: "networkidle2",
    });
    expect(mockPage.waitForSelector).toHaveBeenCalledWith(
      ".org-page-details-module__card-spacing a .link-without-visited-state"
    );
    expect(mockPage.evaluate).toHaveBeenCalledTimes(1);
    expect(mockPage.goto).toHaveBeenCalledWith(
      "https://www.linkedin.com/logout"
    );
    expect(mockPage.goto).toHaveBeenCalledWith(websiteURL, {
      waitUntil: "networkidle2",
    });
